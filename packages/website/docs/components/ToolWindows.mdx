...
import { EmbeddedStory } from "../../src/components/EmbeddedStory";
...
Use `MultiContentToolWindow.Content` in a `MultiContentToolWindow`, to render many views. Each view must have a unique
key.
You can control the active view of a `MultiContentToolWindow` through `activeKey`, and `onActiveKeyChange` props.
Or you can leave it uncontrolled and set `defaultActiveKey` if needed.

```tsx
<MultiContentToolWindow>
  <MultiContentToolWindow.Content tabContent={<>First view</>} key="v1">
    ...
  </MultiContentToolWindow.Content>
  <MultiContentToolWindow.Content tabContent={<>Second view</>} key="v2">
    ...
  </MultiContentToolWindow.Content>
</MultiContentToolWindow>
```
...
#### Multi content tool window header

Similar to [DefaultToolWindow](#defaulttoolwindow), `additionalActions` and `headerContent` props can be used to
define the content that goes into the header of a multi content tool window. However, content switcher UI (whether
it be tabs or dropdown) is rendered immediately after `headerContent`.
If you need more flexibility, you can pass a render function to `headerContent`. Rendered content switcher
...
<MultiContentToolWindow
  headerContent={({ renderedContentSwitcher }) => (
    <>
      ...
      {executions.length > 1 ? renderedContentSwitcher : <SomethingElse />}
    </>
  )}
>
...
#### MultiContentToolWindow story

<EmbeddedStory storyId="components-toolwindow--multi-content" height={450} />

## ToolWindowsState

Use `ToolWindowsState` to create immutable state of a number of tool windows. It accepts a mapping from ids to
