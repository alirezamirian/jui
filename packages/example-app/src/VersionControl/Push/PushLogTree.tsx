...
const toggleSelectionCallback = (get: Getter, set: Setter, value: string) => {
...
// TODO: fix state being remembered
    ...
    <ItemLayout style={{ flex: 1 }}>
      ...
      {!isEditingTargetBranch && (
        <>
          ...
          {target.isNew && <StyledBadge>New</StyledBadge>}
        </>
      )}
      {isEditingTargetBranch && (
        <StyledInput
...
function SetRemoteMenu(
  ...
  const setPushRemote = useSetAtom(pushTargetRemoteAtom(repoPath));
  const onAction = (key: Key) => {
    if (key === "addRemote") {
      notImplemented();
    } else {
      setPushRemote(`${key}`.split(":").slice(1).join(":"));
    }
  };
  return (
